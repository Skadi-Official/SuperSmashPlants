...
extern IMAGE img_gravestone_left;						// 朝左的墓碑图片
extern IMAGE img_gravestone_right;						// 朝右的墓碑图片
...
public:
	...
	void on_update(int delta)
	{
		...
		selector_background_scroll_offset_x += 1; // 每次让线条向右移动五个像素的距离
...
private:
	...
	LPCTSTR str_peashooter_name = _T("豌豆射手");	// 豌豆射手角色名字
	LPCTSTR str_sunflower_name = _T("向日葵");		// 向日葵角色名字

	int selector_background_scroll_offset_x = 0;	// 背景板滚动距离，竖直线条在水平方向上滚动的距离
...
private:
	void outtextxy_shaded(int x, int y, LPCTSTR str)	// 绘制带阴影的字体
	{
